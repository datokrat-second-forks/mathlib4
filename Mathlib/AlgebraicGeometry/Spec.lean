...
The two `IsPrime` types are not equal at `.instances` (neither `Ideal.comap` nor `PrimeSpectrum.comap`
unfolds there), so the direct `checkTypes` fails (`❌`). Under `"mark"` that ends the assignment and
synthesis fails outright; under `"markOrSynth"` the fallback re-synthesizes the instance at the mvar's
type (`✅`, truncated below) and the assignment goes through — which is why the real declaration above
compiles. -/
