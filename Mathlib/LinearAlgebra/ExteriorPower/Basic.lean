...
noncomputable def relationsSolutionEquiv {ι : Type*} [DecidableEq ι] {M : Type*}
  ...
  -- In the proofs below, `relations_G`/`relations_R` must not be used by `dsimp`: rewriting the
  -- *types* `(relations R ι M).G/.R` to their unfoldings would strand instance arguments at the
  -- un-rewritten form, and `show` re-types the goal head at `ι → M` for the same reason
  -- (lean4#9077 instance-typed metavariable check).
  toFun s :=
    { toFun := fun m ↦ s.var m
      map_update_add' := fun m i x y ↦ by
        have := s.linearCombination_var_relation (.add m i x y)
        dsimp [-relations_G, -relations_R] at this ⊢
    ...
    { var := fun m ↦ f m
      linearCombination_var_relation := by
        rintro (⟨m, i, x, y⟩ | ⟨m, i, r, x⟩ | ⟨v, i, j, hm, hij⟩) <;>
          show Finsupp.linearCombination R (fun m ↦ f m) _ = 0
        · simp
        · simp
