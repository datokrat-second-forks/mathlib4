...
theorem min_inv_inv' (a b : α) : min a⁻¹ b⁻¹ = (max a b)⁻¹ :=
  Eq.symm <| (OrderDual.toDual_inj (b := min a⁻¹ b⁻¹)).1 <|
    (@Monotone.map_max α αᵒᵈ _ _ (fun a ↦ OrderDual.toDual a⁻¹) a b) fun _ _ =>
      inv_le_inv_iff.mpr

@[to_additive max_neg_neg]
theorem max_inv_inv' (a b : α) : max a⁻¹ b⁻¹ = (min a b)⁻¹ :=
  Eq.symm <| (OrderDual.toDual_inj (b := max a⁻¹ b⁻¹)).1 <|
    (@Monotone.map_min α αᵒᵈ _ _ (fun a ↦ OrderDual.toDual a⁻¹) a b) fun _ _ =>
      inv_le_inv_iff.mpr
