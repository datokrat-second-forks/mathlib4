...
@[to_additive] lemma inv_sup (a b : α) : (a ⊔ b)⁻¹ = a⁻¹ ⊓ b⁻¹ :=
  (OrderDual.toDual_inj (b := a⁻¹ ⊓ b⁻¹)).1 ((OrderIso.inv α).map_sup _ _)

@[to_additive] lemma inv_inf (a b : α) : (a ⊓ b)⁻¹ = a⁻¹ ⊔ b⁻¹ :=
  (OrderDual.toDual_inj (b := a⁻¹ ⊔ b⁻¹)).1 ((OrderIso.inv α).map_inf _ _)

@[to_additive]
lemma div_sup (a b c : α) : c / (a ⊔ b) = c / a ⊓ c / b :=
  (OrderDual.toDual_inj (b := c / a ⊓ c / b)).1 ((OrderIso.divLeft c).map_sup _ _)

@[to_additive]
lemma div_inf (a b c : α) : c / (a ⊓ b) = c / a ⊔ c / b :=
  (OrderDual.toDual_inj (b := c / a ⊔ c / b)).1 ((OrderIso.divLeft c).map_inf _ _)
