...
reproduce the build failure rather than the CLI default.)

After `dsimp` and the `leftShift_v`/`smul_v`/`Linear.comp_smul` rewrites, the goal is
`u • x • f = x • u • f`, where `x : R`, `f := (K.shiftFunctorObjXIso a p (p + a) ⋯).hom ≫
γ.v (p + a) q ⋯` has type `((shiftFunctor C a).obj K).X p ⟶ L.X q`, and
`u := (a * n' + a * (a - 1) / 2).negOnePow : ℤˣ` is the *sign* that `leftShift` carries (its
definition multiplies by `negOnePow`; `rightShift` involves no signs). To swap the two scalar
actions, `simp` fires `smul_comm x`, which forces synthesis of
`SMulCommClass R ℤˣ (K.X (p + a) ⟶ L.X q)` — the hom-type is spelled with the *reduced* object
`K.X (p + a)` coming from `leftShift_v`'s normal form, while the goal's own `x • f` carries a
`SMul R _` instance spelled at the *shift* object `((shiftFunctor C a).obj K).X p`.

The only viable candidate, `@Units.smulCommClass_right` (`SMulCommClass M Nˣ α` from
`SMulCommClass M N α`), has to reproduce its `[SMul M α]` argument in an instance-typed mvar,
and unifying its conclusion with the goal assigns that mvar the goal's shift-spelled value:
`❌ (?m : SMul R (K.X (p + a) ⟶ L.X q))`
`     := (DistribMulAction.toDistribSMul.toSMul`
`         : SMul R (((shiftFunctor (CochainComplex C ℤ) a).obj K).X p ⟶ L.X q))`.
The two types differ only in the domain object; equating `((shiftFunctor C a).obj K).X p` with
`K.X (p + a)` unfolds `CochainComplex.shiftFunctor`'s object action, which is semireducible and
does **not** reduce at `.instances` (third demo: `with_reducible rfl` fails, `rfl` succeeds), so
the direct `.instances`-transparency type check rejects the assignment.

Under `markOrSynth` the synth fallback then runs but *also* fails — and it is the fallback that
does the damage. It synthesizes an instance for the mvar's own type `SMul R (K.X (p + a) ⟶ L.X q)`
— which succeeds (second demo's `#synth` returns `DistribMulAction.toDistribSMul.toSMul`) — and
then requires the candidate value `v` to be definitionally equal to that synthesized instance.
But `v` and the synthesized instance are the *same* instance `DistribMulAction.toDistribSMul.toSMul`
differing only in the carrier object (`((shiftFunctor C a).obj K).X p` vs `K.X (p + a)`), and this
defeq check runs during instance resolution at `.instances` transparency, where — exactly as the
direct check — those two objects are not defeq. So the candidate is "not definitionally equal to
the synthesized instance" and the assignment fails. This is the `markOrSynth` leg-(c) failure, not
a synthesis-impossible one: the class is perfectly synthesizable, the value simply cannot be
recovered from synthesis at `.instances`. (In the first demo the `…` marks the fallback subtree,
elided by the postprocessor.)

Consequently `Units.smulCommClass_right` is discarded, no other candidate matches,
`SMulCommClass R ℤˣ (K.X (p + a) ⟶ L.X q)` is unsynthesizable, `smul_comm x` never fires (the
`unusedSimpArgs` linter, silenced in the demo, even flags it), and the lemma is left with the
unsolved `u • x • f = x • u • f`.

* The adjacent `rightShift_smul` survives with only `respectTransparency false` because its simp
  set is `[rightShift_v, smul_v, Linear.smul_comp]` with **no** `smul_comm`: `rightShift` carries
  no sign, so there is no `ℤˣ` action to commute past `x`, hence no `SMulCommClass R ℤˣ _`
  synthesis and no instance-typed-mvar rejection. `rightShift_smul` still needs
  `respectTransparency false` because its own rewrite lemmas match across the same shift-functor
  object spelling at `.implicit`, but that milder binder-transparency lenience is a different
  check; `leftShift_smul` needs it too, plus `instanceTypes "none"` for the sign's `SMulCommClass`.
* The rejected mvar is `SMul R _`, a data-carrying (`Type`-valued) instance, so the parked
  Prop-exemption would not apply, even though `SMulCommClass` itself is a `Prop`. Same shape as
  the `Module`-vs-`CompatibleSMul` rejection in `Mathlib/RingTheory/Kaehler/JacobiZariski.lean`.

Possible fixes (proof-local preferred): commute the sign with dedicated category-linearity
lemmas (`Linear.units_smul_comp`/`Linear.comp_units_smul`) instead of the generic `smul_comm`;
or `change` the goal to the reduced object spelling before the `simp only` so the goal's `SMul R`
instance is spelled `K.X (p + a)` and matches the mvar type at `.instances`; or, upstream, make
`CochainComplex.shiftFunctor`'s object action reducible at `.instances` (a definitional change
with broad impact, not proof-local).
-/
...
/-- Truncate every trace subtree below `depth`, marking cut nodes with `…`. -/
private meta partial def maxDepth (depth : Nat) : TracePostprocessor := fun trees =>
  let rec go (t : TraceTree) (d : Nat) : TraceTree :=
    match t with
    | .leaf msg => .leaf msg
    | .node data msg children wrap =>
      match d with
      | 0 => if children.isEmpty then .node data msg #[] wrap
             else .node data m!"{msg} …" #[] wrap
...
-- fallback synthesis (which succeeds, yet whose result is not defeq to the candidate here).
/--
...
postprocess_traces
  filterSubtrees (fun x => (ofClass `Meta.synthInstance.apply x)
      <&&> (containsString "smulCommClass_right" x))
    >=> maxDepth 3
    >=> filterSubtrees (fun x => (failed x) <&&> (ofClass `Meta.isDefEq.assign.checkTypes x)
      <&&> (containsString "DistribMulAction.toDistribSMul.toSMul" x))
in
...
-- The mvar's own type is synthesizable in isolation, so the fallback's synthesis (leg b) succeeds;
-- the failure is leg c, this synthesized value not being defeq to the goal's shift-spelled one.
/-- info: DistribMulAction.toDistribSMul.toSMul -/
#guard_msgs in
#synth SMul R (K.X 0 ⟶ L.X 0)

-- The two object spellings agree at `.default` but not at `.instances`/reducible: unfolding
-- `((shiftFunctor C a).obj K).X p` to `K.X (p + a)` needs the semireducible shift functor. This is
-- why both the direct check and the fallback's `.instances`-level recheck reject the candidate.
example (a p q : ℤ) :
    (K.X (p + a) ⟶ L.X q) = (((shiftFunctor C a).obj K).X p ⟶ L.X q) := by
