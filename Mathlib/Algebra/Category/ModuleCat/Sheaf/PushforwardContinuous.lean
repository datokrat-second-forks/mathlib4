...
lemma pushforwardCompHom_pushforwardIdHom_left [Functor.IsContinuous (𝟭 C ⋙ F) J K] :
...
lemma pushforwardCompHom_pushforwardIdHom_right [Functor.IsContinuous (F ⋙ 𝟭 D) J K] :
...
lemma pushforward_comp_id [Functor.IsContinuous (𝟭 C ⋙ F) J K] :
...
lemma pushforward_id_comp [Functor.IsContinuous (F ⋙ 𝟭 D) J K] :
...
def pushforwardPushforwardAdj : pushforward.{v} φ ⊣ pushforward.{v} ψ where
  ...
  left_triangle_components X := by
    ext U x
    exact (X.val.map_comp_apply (adj.counit.app (F.obj U.unop)).op
      (F.map (adj.unit.app U.unop)).op x).symm.trans
      ((X.val.congr_map_apply (show (adj.counit.app (F.obj U.unop)).op ≫
        (F.map (adj.unit.app U.unop)).op = 𝟙 _ by simp [← op_comp]) x).trans
        (X.val.map_id_apply _ x))
  right_triangle_components X := by
    ext U x
    exact (X.val.map_comp_apply (G.map (adj.counit.app U.unop)).op
      (adj.unit.app (G.obj U.unop)).op x).symm.trans
      ((X.val.congr_map_apply (show (G.map (adj.counit.app U.unop)).op ≫
        (adj.unit.app (G.obj U.unop)).op = 𝟙 _ by simp [← op_comp]) x).trans
        (X.val.map_id_apply _ x))
