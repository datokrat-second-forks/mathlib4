...
theorem polar_iUnion {ι} {s : ι → Set E} : B.polar (⋃ i, s i) = ⋂ i, B.polar (s i) := by
  have h := B.polar_gc.l_iSup (f := s)
  simp only [Function.comp_apply] at h
  rwa [← toDual_iInf, OrderDual.toDual_inj] at h

@[simp]
theorem polar_union {s t : Set E} : B.polar (s ∪ t) = B.polar s ∩ B.polar t := by
  have h := B.polar_gc.l_sup (a₁ := s) (a₂ := t)
  simp only [Function.comp_apply] at h
  rwa [← toDual_inf, OrderDual.toDual_inj] at h
...
theorem polar_empty : B.polar ∅ = Set.univ := by
  have h := B.polar_gc.l_bot
  simp only [Function.comp_apply] at h
  rwa [show (⊥ : (Set F)ᵒᵈ) = OrderDual.toDual ⊤ from rfl, OrderDual.toDual_inj] at h
