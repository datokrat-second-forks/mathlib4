...
/-! #### Realizing a vector bundle core on a family of fibers

Analogously to `FiberBundleCore.localTrivAlong`, a vector bundle core defines a vector bundle
structure on any family of topological modules `V : B → Type*` equipped with continuous linear
fiberwise coordinates `ψ x : V x ≃L[R] F`: the local trivializations are those of the core, read
in each fiber through `ψ`. The bundle `Z.Fiber` constructed above is the realization along the
identity coordinates, and the tangent bundle of a manifold is the realization of
`tangentBundleCore` along the canonical identification of each tangent space with the model
space. -/

variable {V : B → Type*} [∀ x, AddCommMonoid (V x)] [∀ x, Module R (V x)]
  [∀ x, TopologicalSpace (V x)] (ψ : ∀ x, V x ≃L[R] F)

/-- Topology on the total space of the realization of the core `Z` on the family `V` along the
fiberwise coordinates `ψ`. See `VectorBundleCore.toTopologicalSpace` for the realization on
`Z.Fiber`. -/
@[instance_reducible]
def totalSpaceTopologyAlong : TopologicalSpace (Bundle.TotalSpace F V) :=
  Z.toFiberBundleCore.totalSpaceTopologyAlong fun x ↦ (ψ x).toHomeomorph

/-- The realized local trivialization with index `i` of the realization of the core `Z` on the
family `V` along `ψ`. See `VectorBundleCore.localTriv` for the realization on `Z.Fiber`. -/
def localTrivAlong (i : ι) : @Trivialization B F _ _ _ (Z.totalSpaceTopologyAlong ψ) (π F V) :=
  Z.toFiberBundleCore.localTrivAlong (fun x ↦ (ψ x).toHomeomorph) i

/-- Preferred realized local trivialization at a point, for the realization of the core `Z` on
the family `V` along `ψ`. See `VectorBundleCore.localTrivAt` for the realization on
`Z.Fiber`. -/
def localTrivAtAlong (b : B) : @Trivialization B F _ _ _ (Z.totalSpaceTopologyAlong ψ) (π F V) :=
  Z.localTrivAlong ψ (Z.indexAt b)

theorem localTrivAlong_apply (i : ι) (p : Bundle.TotalSpace F V) :
    Z.localTrivAlong ψ i p = ⟨p.1, Z.coordChange (Z.indexAt p.1) i p.1 (ψ p.1 p.2)⟩ :=
  rfl

theorem localTrivAlong_symm_fst (i : ι) (p : B × F) :
    letI := Z.totalSpaceTopologyAlong ψ
    (Z.localTrivAlong ψ i).toOpenPartialHomeomorph.symm p =
      ⟨p.1, (ψ p.1).symm (Z.coordChange i (Z.indexAt p.1) p.1 p.2)⟩ :=
  rfl

theorem mem_localTrivAlong_source (i : ι) (p : Bundle.TotalSpace F V) :
    letI := Z.totalSpaceTopologyAlong ψ
    (p ∈ (Z.localTrivAlong ψ i).source ↔ p.1 ∈ Z.baseSet i) :=
  Iff.rfl

theorem baseSet_localTrivAlong (i : ι) :
    letI := Z.totalSpaceTopologyAlong ψ
    (Z.localTrivAlong ψ i).baseSet = Z.baseSet i :=
  rfl

/-- The realized local trivializations of a vector bundle core along continuous linear fiberwise
coordinates are linear. -/
instance localTrivAlong.isLinear (i : ι) :
    letI := Z.totalSpaceTopologyAlong ψ
    (Z.localTrivAlong ψ i).IsLinear R :=
  letI := Z.totalSpaceTopologyAlong ψ
  { linear := fun b _ =>
      { map_add := fun v w => by
          simp only [localTrivAlong_apply, map_add, mfld_simps]
        map_smul := fun c v => by
          simp only [localTrivAlong_apply, map_smul, mfld_simps] } }

theorem localTrivAlong_coordChange_eq (i j : ι) {b : B} (hb : b ∈ Z.baseSet i ∩ Z.baseSet j)
    (v : F) :
    letI := Z.totalSpaceTopologyAlong ψ
    (Z.localTrivAlong ψ i).coordChangeL R (Z.localTrivAlong ψ j) b v = Z.coordChange i j b v := by
  let := Z.totalSpaceTopologyAlong ψ
  rw [Trivialization.coordChangeL_apply', localTrivAlong_symm_fst, localTrivAlong_apply,
    ContinuousLinearEquiv.apply_symm_apply, coordChange_comp]
  exacts [⟨⟨hb.1, Z.mem_baseSet_at b⟩, hb.2⟩, hb]

/-- The realization of a vector bundle core on a family of fibers along continuous fiberwise
coordinates is a fiber bundle. See `VectorBundleCore.fiberBundle` for the realization on
`Z.Fiber`. -/
@[instance_reducible]
def fiberBundleAlong :
    letI := Z.totalSpaceTopologyAlong ψ
    FiberBundle F V :=
