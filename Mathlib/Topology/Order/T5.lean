...
/-- The mirror image of `compl_ordConnectedSection_ordSeparatingSet_mem_nhdsGE`. It cannot be
obtained from that lemma by passing to `Xᵒᵈ`: `ordConnectedSection` picks a representative of each
order connected component with `Nonempty.some`, and the choice made on `Xᵒᵈ` is a different one. -/
theorem compl_ordConnectedSection_ordSeparatingSet_mem_nhdsLE (hd : Disjoint s (closure t))
  ...
  · simp only [Set.disjoint_left, not_forall, Classical.not_not] at H
    ...
    have hd : Disjoint s (ordConnectedSection (ordSeparatingSet s t)) :=
      disjoint_left_ordSeparatingSet.mono_right ordConnectedSection_subset
    replace hca : c < a := hca.lt_of_ne <| ne_of_mem_of_not_mem hc <| disjoint_left.1 hd ha
