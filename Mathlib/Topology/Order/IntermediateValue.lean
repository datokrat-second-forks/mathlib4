...
theorem IsClosed.mem_of_ge_of_forall_exists_lt {a b : α} {s : Set α} (hs : IsClosed (s ∩ Icc a b))
  ...
  have c_ge : a ≤ c := le_csInf ⟨_, hb⟩ fun x hx => hx.2.1
  rcases eq_or_lt_of_le c_ge with hc | hc
  · exact hc ▸ c_mem.1
  exfalso
  rcases hgt c ⟨c_mem.1, hc, c_mem.2.2⟩ with ⟨x, xs, ax, xc⟩
  exact not_lt_of_ge (csInf_le Sbd ⟨xs, ax, le_trans xc.le (csInf_le Sbd hb)⟩) xc
...
theorem ContinuousOn.strictAntiOn_of_injOn_Icc {a b : α} {f : α → δ}
  ...
  ContinuousOn.strictMonoOn_of_injOn_Icc (δ := δᵒᵈ) hab hfab
    (continuous_toDual.comp_continuousOn hf_c)
    (fun _ hx _ hy h ↦ hf_i hx hy (OrderDual.toDual.injective h))
