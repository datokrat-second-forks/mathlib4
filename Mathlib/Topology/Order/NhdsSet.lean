...
theorem hasBasis_nhdsSet_Ici_Ioi (a : α) [h : Nonempty (Iio a)] :
  ...
  exact ⟨b, hab, fun x hx ↦ (le_or_gt x a).elim (fun hxa ↦ hbs ⟨hx, hxa⟩) fun hxa ↦ hs.2 hxa⟩
...
theorem hasBasis_nhdsSet_Ici_Ici (a : α) [NeBot (𝓝[<] a)] :
  ...
  obtain ⟨x, hcx, hxa⟩ := Filter.nonempty_of_mem (Ioo_mem_nhdsLT hc)
  exact ⟨x, hxa, Ici_subset_Ioi.2 hcx⟩

@[simp]
theorem Ici_mem_nhdsSet_Ici_iff {a b : α} [NeBot (𝓝[<] b)] : Ici a ∈ 𝓝ˢ (Ici b) ↔ a < b :=
  (hasBasis_nhdsSet_Ici_Ici b).mem_iff.trans
    ⟨fun ⟨_c, hcb, hac⟩ ↦ (Ici_subset_Ici.1 hac).trans_lt hcb, fun h ↦ ⟨_, h, Subset.rfl⟩⟩
