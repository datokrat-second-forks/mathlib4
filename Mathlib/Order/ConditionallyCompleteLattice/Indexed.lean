...
attribute [to_dual existing le_ciInf_iff] ciSup_le_iff
...
attribute [to_dual existing] IsLUB.ciSup_eq IsLUB.ciSup_set_eq
...
@[to_dual ciInf_le /-- The indexed infimum of a function is bounded above by the value
taken at one point -/]
...
@[gcongr low, to_dual (attr := gcongr low)
  /-- The indexed infima of two functions are comparable if the functions are pointwise
  comparable -/]
...
attribute [to_dual existing ciInf₂_le] le_ciSup₂
...
attribute [to_dual existing] ciSup_subtype_fun
...
attribute [to_dual existing] cbiSup_of_not_bddAbove
