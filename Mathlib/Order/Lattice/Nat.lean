...
theorem iInf_lt_succ (u : ℕ → α) (n : ℕ) : ⨅ k < n + 1, u k = (⨅ k < n, u k) ⊓ u n := by
  ...
  simpa only [toDual_iInf, toDual_inf] using iSup_lt_succ (fun k => OrderDual.toDual (u k)) n
...
theorem iInf_le_succ (u : ℕ → α) (n : ℕ) : ⨅ k ≤ n + 1, u k = (⨅ k ≤ n, u k) ⊓ u (n + 1) := by
  rw [← OrderDual.toDual_inj]
  simpa only [toDual_iInf, toDual_inf] using iSup_le_succ (fun k => OrderDual.toDual (u k)) n
