...
theorem WellFoundedOn.union (hs : s.WellFoundedOn r) (ht : t.WellFoundedOn r) :
  ...
  rintro f hf
  obtain ⟨g, hg⟩ := Nat.exists_subseq_of_forall_mem_union f hf
  let g' : ((· > ·) : ℕ → ℕ → Prop) ↪r ((· > ·) : ℕ → ℕ → Prop) := ⟨g.toEmbedding, g.lt_iff_lt⟩
  rcases hg with hg | hg
  exacts [hs (RelEmbedding.trans g' f) hg, ht (RelEmbedding.trans g' f) hg]
...
theorem BddAbove.wellFoundedOn_gt : BddAbove s → s.WellFoundedOn (· > ·) := by
  ...
  exact (finite_Icc (f 0) a).subset <| range_subset_iff.2 <| fun n =>
    ⟨monotone_iff_forall_lt.2 (fun a b hab => (f.map_rel_iff.2 hab).le) <| Nat.zero_le _,
      ha <| hf _⟩
