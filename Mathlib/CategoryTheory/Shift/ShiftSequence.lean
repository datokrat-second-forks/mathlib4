...
noncomputable def ShiftSequence.tautological : ShiftSequence F M where
  ...
  shiftIso_add n m a a' a'' ha' ha'' := by
    ...
    simp only [Functor.associator_hom_app, Functor.associator_inv_app, Functor.comp_obj,
      Functor.comp_map, id_comp, ← Functor.map_comp]
    congr
    simpa only [← cancel_epi ((shiftFunctor C a).map ((shiftFunctorAdd C m n).hom.app X)),
      ...
      Functor.map_id, id_comp] using!
      shiftFunctorAdd'_assoc_inv_app m n a (m + n) a' a'' rfl ha'
...
def leftComp [π.CommShift M] [H.ShiftSequence M] : F.ShiftSequence M where
  ...
  shiftIso_add n m a a' a'' ha' ha'':= by
    ext K
    dsimp
    simp only [Functor.associator_hom_app, Functor.associator_inv_app,
      H.shiftIso_add_hom_app n m a a' a'' ha' ha'', assoc, commShiftIso_add,
