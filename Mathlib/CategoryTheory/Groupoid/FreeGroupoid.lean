...
-- Removing `backward.isDefEq.respectTransparency.instances false` failed here too, but for another
-- reason. This declaration runs no `induction`, so it is not the universe problem above. Without
-- the option the last `rw` line reports `Tactic `rewrite` failed: Did not find an occurrence`. That
-- failure is not analysed yet.
-- Fixed by making `Quiver.FreeGroupoid` implicit-reducible.
attribute [local implicit_reducible] Quiver.FreeGroupoid in
