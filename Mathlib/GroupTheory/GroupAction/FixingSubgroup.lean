...
theorem fixedPoints_submonoid_iSup {ι : Sort*} {P : ι → Submonoid M} :
  ...
  have h := (fixingSubmonoid_fixedPoints_gc M α).u_iInf (f := fun i => OrderDual.toDual (P i))
...
theorem fixedPoints_subgroup_iSup {ι : Sort*} {P : ι → Subgroup M} :
  ...
  have key : OrderDual.ofDual (⨅ i, OrderDual.toDual (P i)) = ⨆ i, P i := by
    rw [← toDual_iSup, OrderDual.ofDual_toDual]
  have h := (fixingSubgroup_fixedPoints_gc M α).u_iInf (f := fun i => OrderDual.toDual (P i))
  simp only [Function.comp_apply, OrderDual.ofDual_toDual] at h
  exact (congrArg (fun Q : Subgroup M => fixedPoints Q α) key.symm).trans h
