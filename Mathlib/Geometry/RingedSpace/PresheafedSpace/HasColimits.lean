...
/-- `colimit.w` for the underlying continuous maps of a diagram of presheafed spaces. -/
theorem colimit_w_base (F : J ⥤ PresheafedSpace.{_, _, v} C) {j j' : J} (f : j ⟶ j') :
    (F.map f).base ≫ colimit.ι (F ⋙ PresheafedSpace.forget C) j' =
      colimit.ι (F ⋙ PresheafedSpace.forget C) j := by
  rw [← PresheafedSpace.forget_map, ← Functor.comp_map]
  exact colimit.w _ _
...
def pushforwardDiagramToColimit (F : J ⥤ PresheafedSpace.{_, _, v} C) :
  ...
  map {j j'} f :=
    ((pushforward C (colimit.ι (F ⋙ PresheafedSpace.forget C) j')).map (F.map f).c ≫
      (Pushforward.comp (F.map f).base
        (colimit.ι (F ⋙ PresheafedSpace.forget C) j') (F.obj j).presheaf).inv ≫
      (pushforwardEq (colimit_w_base F f) (F.obj j).presheaf).hom).op
  map_id j := by
  ...
  map_comp {j₁ j₂ j₃} f g := by
    ...
    have :
      ...
      exact Functor.congr_obj (congr_arg Opens.map (colimit_w_base F g)) U.unop
